
#include  <mni.h>

public  Real  map_viewport_to_slice_1d(
    Real viewport_pos,
    Real translation,
    Real scale_factor )
{
    return( (viewport_pos - translation) / scale_factor );
}

public  Real  map_slice_to_viewport_1d(
    Real slice_pos,
    Real translation,
    Real scale_factor )
{
    return( slice_pos * scale_factor + translation );
}

private  void  clip_slice_to_pixels_1d(
    int           viewport_size,
    Real          translation,
    Real          delta,
    Real          n_voxels,
    int           *start_pixel,
    int           *end_pixel )
{
    Real      clipped_start, clipped_end;
    Real      real_start_pixel, real_end_pixel;

    clipped_start = map_viewport_to_slice_1d( -0.5, translation, delta );

    if( clipped_start < -0.5 )
        clipped_start = -0.5;
    else if( clipped_start > (Real) n_voxels - 0.5 )
        clipped_start = (Real) n_voxels - 0.5;

    clipped_end = map_viewport_to_slice_1d( (Real) viewport_size - 0.5,
                                            translation, delta );

    if( clipped_end < -0.5 )
        clipped_end = -0.5;
    else if( clipped_end > (Real) n_voxels - 0.5 )
        clipped_end = (Real) n_voxels - 0.5;

    if( delta > 0.0 && clipped_end > clipped_start ||
        delta < 0.0 && clipped_end < clipped_start )
    {
        real_start_pixel = map_slice_to_viewport_1d( clipped_start,
                                                     translation, delta );
        real_end_pixel = map_slice_to_viewport_1d( clipped_end,
                                                   translation, delta );

        if( IS_INT( real_start_pixel ) )
            *start_pixel = (int) real_start_pixel;
        else
            *start_pixel = (int) real_start_pixel + 1;

        if( IS_INT( real_end_pixel ) )
            *end_pixel = (int) real_end_pixel - 1;
        else
            *end_pixel = (int) real_end_pixel;
    }
    else
    {
        *start_pixel = 0;
        *end_pixel = -1;
    }
}

public  void  clip_slice_to_pixel_range(
    int           x_viewport_size,
    int           y_viewport_size,
    int           x_volume_size,
    int           y_volume_size,
    Real          x_translation,
    Real          y_translation,
    Real          x_delta,
    Real          y_delta,
    int           *x_pixel_start,
    int           *y_pixel_start,
    int           *x_pixel_end,
    int           *y_pixel_end )
{
    clip_slice_to_pixels_1d( x_viewport_size, x_translation, x_delta,
                             x_volume_size, x_pixel_start, x_pixel_end );

    clip_slice_to_pixels_1d( y_viewport_size, y_translation, y_delta,
                             y_volume_size, y_pixel_start, y_pixel_end );
}
